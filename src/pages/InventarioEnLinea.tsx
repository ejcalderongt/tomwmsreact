...
function InventarioEnLinea() {
  ...
  const descargarExcel = async () => {
    ...
    try {
      ...
      // Configurar anchos de columnas (11 columnas de datos + espacio para logo)
      worksheet.columns = [
        { width: 24 },  // A - Código
        { width: 32 },  // B - Producto
        { width: 18 },  // C - Existencia Total
        { width: 14 },  // D - Disponible
        { width: 14 },  // E - Reservado
        { width: 14 },  // F - Lote
        { width: 14 },  // G - Licencia
        { width: 14 },  // H - Referencia
        { width: 14 },  // I - Fecha Vence
        { width: 14 },  // J - Fecha Ingreso
        { width: 18 },  // K - Bodega
        { width: 10 },  // L - Logo
        { width: 10 },  // M - Logo
        { width: 10 }   // N - Logo
      ...
      // Insertar logotipo en J2:L7 (3 columnas x 6 filas, inicia en columna J)
      worksheet.mergeCells('J2:L7');
      ...
      // Fila 9: Título "INVENTARIO" centrado (11 columnas: A-K)
      ...
      todosLosDatos.forEach((item: InventarioItem, index: number) => {
        ...
        row.getCell(3).value = item.cantidad_UMBas || 0;
        row.getCell(4).value = item.disponible_UMBas || 0;
        row.getCell(5).value = item.cantidadReservadaUmBas || 0;
        row.getCell(6).value = item.lote || '';
        row.getCell(7).value = item.licencia || '';
        row.getCell(8).value = item.referencia || '';
        row.getCell(9).value = item.fecha_vence ? formatDate(item.fecha_vence) : '';
        row.getCell(10).value = item.fecha_ingreso ? formatDate(item.fecha_ingreso) : '';
        row.getCell(11).value = item.bodega || '';
        
        // Aplicar bordes a todas las celdas de datos (11 columnas)
        for (let c = 1; c <= 11; c++) {
      ...
      // Aplicar bordes desde fila 9 hasta fila 1000 (11 columnas)
              ...
              <div className="flex-1 overflow-y-auto overflow-x-auto" style={{ overflowX: 'auto', overflowY: 'auto' }}>
                <table className="min-w-full divide-y divide-gray-200" style={{ minWidth: '900px' }}>
                    ...
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Existencia Total</th>
