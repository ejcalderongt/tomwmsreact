...
import { ingresosAPI, polizasAPI } from '@/api/api';
...
function DetalleDocumentoIngreso() {
    ...
    try {
      ...
      switch (index) {
        ...
        case 1:
          ...
          break;
        case 2:
          console.log("clic en poliza");
          await cargarPoliza(id, token);
          break;
      }
    } catch (error) {
  ...
  const cargarPoliza = async (id: number, token: string) => {
    try {
      console.log(`Cargando póliza para ID: ${id}`);
      const data = await polizasAPI.obtenerPoliza(id, token);
      console.log("Respuesta póliza:", data);
      setPoliza(data || null);

      if (!data) {
        toast('No se encontró información de póliza para este documento', {
          icon: 'ℹ️',
          style: {
            background: '#3b82f6',
            color: '#fff'
        ...
        toast.success('Información de póliza cargada correctamente');
      }
    } catch (error) {
        console.error('Error al cargar póliza:', error);
        toast.error('Error al cargar la información de la póliza');
      }
  };
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="space-y-4">
                {!poliza ? (
                  <div className="text-center py-8">
                    <p className="text-gray-500">No se encontró información de póliza para este documento</p>
                  </div>
                ) : (
                  <div className="bg-gray-50 rounded-lg p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Información de Póliza</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-medium text-gray-900">No. Póliza: </span>
                        <span className="text-sm text-gray-700">{poliza.noPoliza}</span>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-900">Fecha: </span>
                        <span className="text-sm text-gray-700">{formatDate(poliza.fecha)}</span>
                      </div>
                      {poliza.observaciones && (
                        <div className="md:col-span-2">
                          <span className="text-sm font-medium text-gray-900">Observaciones: </span>
                          <span className="text-sm text-gray-700">{poliza.observaciones}</span>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="mt-6">
                        ...
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {poliza.detalle.map((item: any, index: number) => (
                                <tr key={index} className="hover:bg-gray-50">
                                  <td className="px-4 py-2 text-sm text-gray-900">{item.concepto || 'N/A'}</td>
                                  <td className="px-4 py-2 text-sm text-gray-900">{item.descripcion || 'N/A'}</td>
                                  <td className="px-4 py-2 text-sm text-gray-900">{item.valor ? formatNumber(item.valor) : 'N/A'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
