...
export class MemStorage implements IStorage {
  ...
  async createProduct(insertProduct: InsertProduct): Promise<Product> {
    ...
    const product: Product = {
      ...
      unitPrice: insertProduct.unitPrice || null,
      minStock: insertProduct.minStock || null,
      maxStock: insertProduct.maxStock || null,
  ...
  async createIncomingOrder(insertOrder: InsertIncomingOrder): Promise<IncomingOrder> {
    ...
    const order: IncomingOrder = {
      ...
      totalItems: insertOrder.totalItems || null,
      receivedItems: insertOrder.receivedItems || null,
  ...
  async createIncomingOrderItem(insertItem: InsertIncomingOrderItem): Promise<IncomingOrderItem> {
    ...
    const item: IncomingOrderItem = {
      ...
      expectedQuantity: insertItem.expectedQuantity,
      receivedQuantity: insertItem.receivedQuantity || null,
  ...
  async createOutgoingOrder(insertOrder: InsertOutgoingOrder): Promise<OutgoingOrder> {
    ...
    const order: OutgoingOrder = {
      ...
      status: insertOrder.status || "pending",
      priority: insertOrder.priority || null,
      totalItems: insertOrder.totalItems || null,
      pickedItems: insertOrder.pickedItems || null,
      createdBy: insertOrder.createdBy || null,
      createdAt: new Date(),
      shippedAt: null,
    };
  ...
  async createOutgoingOrderItem(insertItem: InsertOutgoingOrderItem): Promise<OutgoingOrderItem> {
    ...
    const item: OutgoingOrderItem = {
      ...
      orderId: insertItem.orderId,
      productId: insertItem.productId,
      requestedQuantity: insertItem.requestedQuantity,
      pickedQuantity: insertItem.pickedQuantity || null,
      locationId: insertItem.locationId || null,
      unitPrice: insertItem.unitPrice || null,
      status: insertItem.status || null,
    };
  ...
  async createInventoryMovement(insertMovement: InsertInventoryMovement): Promise<InventoryMovement> {
    ...
    const movement: InventoryMovement = {
      ...
      locationId: insertMovement.locationId,
      movementType: insertMovement.movementType,
      quantity: insertMovement.quantity,
      previousQuantity: insertMovement.previousQuantity,
      newQuantity: insertMovement.newQuantity,
